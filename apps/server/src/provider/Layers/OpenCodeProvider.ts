import type { ModelCapabilities, OpenCodeSettings, ServerProvider } from "@t3tools/contracts";
...
const makePendingOpenCodeProvider = (settings: OpenCodeSettings): ServerProvider => {
...
export function checkOpenCodeProviderStatus(input: {
  ...
  const fallback = (cause: unknown, version: string | null = null) => {
    const installed = !(cause instanceof Error && isCommandMissingCause(cause));
    return buildServerProvider({
      provider: PROVIDER,
